...
u8        *__ah_arg_fuzz;
u8         __ah_current_target_buffer[256];
u8        *__ah_current_target;
u8        __ah_recording = 0;
u32 __afl_final_loc;
...
/* SHM buffer to feed args back to buffer. */

static void __ah_map_arg_shm() {

  u8 *map = NULL;
  char *id_str = getenv(SHM_ARG_FUZZ_ENV_VAR);

  if (__afl_debug) {

    fprintf(stderr, "DEBUG: arg feedback shmem %s\n", id_str ? id_str : "none");
  ...
  while (1) {
    ...
    if (!child_stopped) {
      ...
      if (!child_pid) {
        ...
        // memset(__ah_current_target_buffer, 0, 256);
        // read(FORKSRV_FD, __ah_current_target_buffer, 255);
        // __ah_current_target = __ah_current_target_buffer;
        signal(SIGCHLD, old_sigchld_handler);
        signal(SIGTERM, old_sigterm_handler);
    ...
    // char buf[7];
    // memset(buf, 0, 7);
    // write(FORKSRV_FD + 1, "saludos", 6);
    // read(FORKSRV_FD, buf, 6);
...
__attribute__((constructor())) void __afl_auto_init(void) {
  ...
  read(FORKSRV_FD, &__ah_recording, 1);
