...
struct queue_entry {
  ...
  u8 *targetFunctionName;
  u32 targetFunctionIdx;
...
typedef struct afl_state {
  ...
  u8 skip_deterministic,                /* Skip deterministic stages?       */
      ...
      arg_mode,                         /* Fuzzing only function arguments  */
  ...
  double **multi_alias_probability;     /* multiple alias weighted probabilities     */
  u32    **multi_alias_table;           /* multiple alias weighted random lookup table */
...
#define N_FUZZ_SIZE (1 << 21)
  ...
  u64 total_cal_us[MAX_TARGETS],                     /* Total calibration time (us)      */
      total_cal_cycles[MAX_TARGETS];                 /* Total calibration cycles         */

  u64 total_bitmap_size[MAX_TARGETS],                /* Total bit count for all bitmaps  */
      total_bitmap_entries[MAX_TARGETS];             /* Number of bitmaps counted        */
...
#define FOREIGN_SYNCS_MAX 32U
  ...
  u8 *targetNames[MAX_TARGETS];
  u32 entriesForTarget[MAX_TARGETS];
  u8 targetActive[MAX_TARGETS];
  u32 amountOfTargets;
  u32 nextTarget;
  bool firstCycleDone;
...
/* Setup shmem for arg feedback */
void   init_arg_feedback_channel(afl_state_t *);
...
void add_to_queue(afl_state_t *, u8 *, u32, u8, u8 *, u32);
void add_favored_to_queue(afl_state_t *, u8 *, u32, u8, u8 *, u32);
...
#endif
u8 save_if_interesting(afl_state_t *, void *, u32, u8, u8 *, u32);
...
u32  write_to_testcase(afl_state_t *, void **, u32, u32, u8 *);
...
void   get_targets(afl_state_t *);
void   get_seeds_from_runtime(afl_state_t *);
...
void   cycleTarget(afl_state_t *afl);
u32    select_next_queue_entry_from_next_target(afl_state_t *afl);
u32    select_next_queue_entry_from_all_targets(afl_state_t *afl);
...
inline u32 getTargetIdx(afl_state_t *afl, char *fnName) {
  u32 funIdx = 0;
  for (size_t i = 0; i < afl->amountOfTargets; i++) {
    if (!strcmp(fnName, afl->targetNames[i])) {
      funIdx = i;
      break;
    }
  }
...
__attribute__((always_inline))
inline char * getTargetName(char *fnName) {
  char *separator = strchr(fnName, '-');
  size_t len = separator - fnName;
  char *res = malloc(len + 1);
  memcpy(res, fnName, len);
  res[len] = 0;
  return res;
}
