...
inline struct json_value_s *parse_json_file(const char *path) {

  struct json_value_s *res;
  FILE *f = fopen(path, "r");
  ...
  fseek(f, 0, SEEK_END);
  size_t srcSize = ftell(f);
  fseek(f, 0, SEEK_SET);
  char *src = malloc(srcSize);
  (void)fread(src, 1, srcSize, f);
  fclose(f);

  // Parse the json string
  res = json_parse(src, srcSize);
  free(src);
  return res;
}
