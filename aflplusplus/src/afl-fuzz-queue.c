...
static size_t nextTarget = 0;
static time_t lastTargetSwitch;
static u8 lastTargetSwitchInitialized = 0;

inline void cycleTarget (afl_state_t *afl) {
  do {

    nextTarget = (nextTarget + 1) % afl->amountOfTargets;

  } while (!afl->targetActive[nextTarget]);
}

/* select next queue entry based on alias algo - fast! */

inline u32 select_next_queue_entry_from_next_target(afl_state_t *afl) {
  if (unlikely(!lastTargetSwitchInitialized)) {
    lastTargetSwitch = time(NULL);
    lastTargetSwitchInitialized = 1;
  }

  if ((time(NULL) - lastTargetSwitch) > 60) {

    u32 prevTarget = nextTarget;

    cycleTarget(afl);

    if (prevTarget > nextTarget) {

      // we looped around all the available target, from now on the common algorithm decides which function to fuzz
      afl->firstCycleDone = true;
      // create alias table right away to be on the safe side
      create_single_alias_table(afl);
  ...
  u32    s = rand_below(afl, afl->entriesForTarget[nextTarget]);
  ...
  */
  ...
  u32 res = (p < afl->multi_alias_probability[nextTarget][s] ? s : afl->multi_alias_table[nextTarget][s]);

  u32 j = 0;
  for (u32 i = 0; i < afl->queued_items; i++) {

    if (afl->queue_buf[i]->targetFunctionIdx == nextTarget) {
...
inline u32 select_next_queue_entry_from_all_targets(afl_state_t *afl) {
  ...
  afl->alias_probability[s] ? s : afl->alias_table[s]);
  */
  u32 res = (p < afl->alias_probability[s] ? s : afl->alias_table[s]);
...
inline u32 select_next_queue_entry(afl_state_t *afl) {
  ...
  if (!afl->firstCycleDone) {

    res = select_next_queue_entry_from_next_target(afl);

  } else {

    res = select_next_queue_entry_from_all_targets(afl);
...
void create_multi_alias_table(afl_state_t *afl) {

  if (!afl->multi_alias_table) {

    afl->multi_alias_table = malloc(MAX_TARGETS * sizeof(void*));
    memset(afl->multi_alias_table, 0, MAX_TARGETS * sizeof(void*));

  }

  if (!afl->multi_alias_probability) {

    afl->multi_alias_probability = malloc(MAX_TARGETS * sizeof(void*));
    memset(afl->multi_alias_probability, 0, MAX_TARGETS * sizeof(void*));

  }

  for (size_t target = 0; target < MAX_TARGETS; target++) {

    u32    n = afl->entriesForTarget[target], i = 0, j = 0, a, g;
    double sum = 0;

    afl->multi_alias_table[target] =
        (u32 *)afl_realloc((void **)&afl->multi_alias_table[target], n * sizeof(u32));
    afl->multi_alias_probability[target] = (double *)afl_realloc(
        (void **)&afl->multi_alias_probability[target], n * sizeof(double));
    ...
    if (!P || !S || !L || !afl->multi_alias_table[target] || !afl->multi_alias_probability[target]) {

      FATAL("could not acquire memory for alias table");

    }

    memset((void *)afl->multi_alias_table[target], 0, n * sizeof(u32));
    memset((void *)afl->multi_alias_probability[target], 0, n * sizeof(double));
    ...
    if (likely(afl->schedule < RARE)) {
      ...
      for (i = 0; i < afl->queued_items; i++) {
        ...
        if (likely(!q->disabled) && target == q->targetFunctionIdx) {
      ...
      for (i = 0, j = 0; i < afl->queued_items; i++) {

        struct queue_entry *q = afl->queue_buf[i];

        // weight is always 0 for disabled entries
        if (target == q->targetFunctionIdx) {
    ...
    while (nS && nL) {
      ...
      a = S[--nS];
      g = L[--nL];
      afl->multi_alias_probability[target][a] = P[a];
      afl->multi_alias_table[target][a] = g;
      P[g] = P[g] + P[a] - 1;
      if (P[g] < 1) {
    ...
    while (nL)
      afl->multi_alias_probability[target][L[--nL]] = 1;

    while (nS)
      afl->multi_alias_probability[target][S[--nS]] = 1;
...
void create_single_alias_table(afl_state_t *afl) {
...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det, u8 *funName, u32 funIdx) {
  ...
  q->targetFunctionName = strdup(funName);
  q->targetFunctionIdx = funIdx;
  afl->entriesForTarget[funIdx]++;
...
void add_favored_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det, u8 *funName, u32 funIdx) {

  add_to_queue(afl, fname, len, passed_det, funName, funIdx);
  afl->queue_buf[afl->queued_items - 1]->favored = true;
  ++afl->queued_favored;
  ++afl->pending_favored;
...
void cull_queue(afl_state_t *afl) {
  ...
  for (i = 0; i < afl->queued_items; i++) {
    ...
    // We want to force the fuzzing of the initial seeds if we are still doing our first round around the queue.
    // Way too hacky, think of something better.
    if (!afl->firstCycleDone) {

        // Let's also tag as favored every initial seed in the queue
        if (i < afl->amountOfTargets) {

            afl->queue_buf[i]->fuzz_level = 0;
            afl->queue_buf[i]->was_fuzzed = false;
            ...
            if (!afl->queue_buf[i]->favored) {
                ...
                if (!afl->queue_buf[i]->was_fuzzed) { ++afl->pending_favored; }
...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  u32 avg_exec_us = afl->total_cal_us[q->targetFunctionIdx] / afl->total_cal_cycles[q->targetFunctionIdx];
  u32 avg_bitmap_size = afl->total_bitmap_size[q->targetFunctionIdx] / afl->total_bitmap_entries[q->targetFunctionIdx];
  u32 perf_score = 100;
