...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault, u8 *target, u32 funIdx) {
  ...
  save_to_queue:
    ...
    queue_fn =
        alloc_printf("%s/queue/%s-id:%06u,%s", afl->out_dir, target, afl->queued_items,
    ...
    queue_fn =
        alloc_printf("%s/queue/%s-id_%06u", afl->out_dir, target, afl->queued_items);
    ...
    close(fd);
    add_to_queue(afl, queue_fn, len, 0, target, funIdx);
  ...
  switch (fault) {
    ...
    case FSRV_RUN_TMOUT:
      ...
      if (afl->fsrv.exec_tmout < afl->hang_tmout) {
        ...
        u8  new_fault;
        u32 tmp_len = write_to_testcase(afl, &mem, len, 0, target);
        ...
        if (likely(tmp_len)) {
          ...
          len = write_to_testcase(afl, &mem, len, 1, target);
      ...
      snprintf(fn, PATH_MAX, "%s/hangs/%s-id:%06llu,%s", afl->out_dir, target,
               afl->saved_hangs,
               describe_op(afl, 0, NAME_MAX - strlen("id:000000,")));

#else

      snprintf(fn, PATH_MAX, "%s/hangs/%s-id_%06llu", afl->out_dir, target,
               afl->saved_hangs);
    ...
    keep_as_crash:
      ...
      snprintf(fn, PATH_MAX, "%s/crashes/%s-id:%06llu,sig:%02u,%s", afl->out_dir, target,
               afl->saved_crashes, afl->fsrv.last_kill_signal,
               describe_op(afl, 0, NAME_MAX - strlen("id:000000,sig:00,")));

#else

      snprintf(fn, PATH_MAX, "%s/crashes/%s-id_%06llu_%02u", afl->out_dir, target,
               afl->saved_crashes, afl->fsrv.last_kill_signal);
