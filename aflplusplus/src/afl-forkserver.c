...
void afl_fsrv_start(afl_forkserver_t *fsrv, char **argv,
  ...
  write(fsrv->fsrv_ctl_fd, &fsrv->recordArgs, 1);

  if (!fsrv->targetFunctionName) {

    write(fsrv->fsrv_ctl_fd, "__AH_NON_EXISTENT__", strlen("__AH_NON_EXISTENT__"));

  } else {

    write(fsrv->fsrv_ctl_fd, fsrv->targetFunctionName, strlen(fsrv->targetFunctionName));
...
afl_fsrv_run_target(afl_forkserver_t *fsrv, u32 timeout,
  ...
  // u8 *targetName = fsrv->targetFunctionName;
  // (void*)write(fsrv->fsrv_ctl_fd, targetName, strlen(targetName));
  ...
  // char buf[7];
  // memset(buf, 0, 7);
  // int r = read(fsrv->fsrv_st_fd, buf, 6);
  // printf("parent got %d bytes: %s\n", r, buf);
  // write(fsrv->fsrv_ctl_fd, "saludos", 6);
#ifdef AFL_PERSISTENT_RECORD
  ...
  bool ignore_aborts = getenv("AFL_IGNORE_ABORT") != 0;

  if (unlikely(
          /* A normal crash/abort */
          ((WIFSIGNALED(fsrv->child_status)) && (!ignore_aborts || WTERMSIG(fsrv->child_status) != SIGABRT)) ||
