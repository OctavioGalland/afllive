...
static void edit_params(u32 argc, char **argv, char **envp) {
  ...
  if (compiler_mode == LLVM || compiler_mode == LTO) {
    ...
    const char *ahRoot = NULL;
    const char *ahConfig = getenv("AH_CONFIG");
    if (ahConfig) {
      struct json_value_s *config = parse_json_file(ahConfig);
      ...
      for (struct json_object_element_s *k = config_root->start; k != NULL; k = k->next) {
        ...
        if (!strcmp(k->name->string, "root")) {

          ahRoot = json_value_as_string(k->value)->string;
      ...
      if (ahRoot != NULL) {
        ...
        cc_params[cc_par_cnt++] = "-lgen";
        cc_params[cc_par_cnt++] = alloc_printf("-L%s/libgen", ahRoot);
        cc_params[cc_par_cnt++] = alloc_printf("-Wl,-rpath=%s/libgen", ahRoot);
        cc_params[cc_par_cnt++] = alloc_printf("-fpass-plugin=%s/instrumentation/pass.so", ahRoot);

      } else {

        WARNF("[AH] Autoharness root folder not specified, skipping instrumentation!");
      ...
      WARNF("[AH] Config file not specified, skipping instrumentation!");
