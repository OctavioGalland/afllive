...
write_to_testcase(afl_state_t *afl, void **mem, u32 len, u32 fix, u8 *target) {

  // if (afl->fsrv.targetFunctionName) {
  //   free(afl->fsrv.targetFunctionName);
  // }
  // afl->fsrv.targetFunctionName = strdup(target);
...
u8 calibrate_case(afl_state_t *afl, struct queue_entry *q, u8 *use_mem,
  ...
  u8 restartNeeded = !afl->fsrv.targetFunctionName || strcmp(afl->fsrv.targetFunctionName, q->targetFunctionName);
  be_quiet = 1;
  if (afl->fsrv.fsrv_pid && restartNeeded) {
  ...
  if (!afl->fsrv.fsrv_pid || restartNeeded) {
    afl->fsrv.targetFunctionName = q->targetFunctionName;
  ...
  if (unlikely(afl->shm.cmplog_mode)) {
    ...
    (void)write_to_testcase(afl, (void **)&use_mem, q->len, 1, q->targetFunctionName);
  ...
  afl->total_cal_us[q->targetFunctionIdx] += diff_us;
  afl->total_cal_cycles[q->targetFunctionIdx] += afl->stage_max;
  ...
  afl->total_bitmap_size[q->targetFunctionIdx] += q->bitmap_size;
  ++(afl->total_bitmap_entries[q->targetFunctionIdx]);
  ...
  while ((sd_ent = readdir(sd))) {
    ...
    for (o = m; o < n; o++) {
      ...
      if (st.st_size && st.st_size <= MAX_FILE) {
        ...
        char *targetName = getTargetName(namelist[o]->d_name);
        (void)write_to_testcase(afl, (void **)&mem, st.st_size, 1, targetName);
        ...
        afl->queued_imported +=
            save_if_interesting(afl, mem, st.st_size, fault, targetName, getTargetIdx(afl, targetName));
        afl->syncing_party = 0;
        free(targetName);
...
common_fuzz_stuff(afl_state_t *afl, u8 *out_buf, u32 len) {
  ...
  if (unlikely(len = write_to_testcase(afl, (void **)&out_buf, len, 0, afl->queue_cur->targetFunctionName)) == 0) {
  ...
  afl->queued_discovered += save_if_interesting(afl, out_buf, len, fault, afl->queue_cur->targetFunctionName, afl->queue_cur->targetFunctionIdx);
