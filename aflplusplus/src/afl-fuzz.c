...
static void at_exit() {
  ...
  char *list[5] = {SHM_ENV_VAR, SHM_FUZZ_ENV_VAR, SHM_ARG_FUZZ_ENV_VAR, CMPLOG_SHM_ENV_VAR, NULL};
  char *ptr;
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (
      (opt = getopt(
           argc, argv,
           "a+Ab:B:c:CdDe:E:hi:I:f:F:g:G:l:L:m:M:nNOo:p:RQs:S:t:T:UV:WXx:YZ")) >
      0) {
  ...
  // TODO: since we broke non-arg fuzzing functionality we'll keep this here for now
  afl->arg_mode = 1;
  ...
  if (!afl->arg_mode && !strcmp(afl->in_dir, afl->out_dir)) {
  ...
  if (afl->arg_mode == 1 && afl->in_place_resume) {
    // TODO: this breaks functionality when resuming non in-vivo fuzzing campaigns
  ...
  if (afl->shmem_testcase_mode) {
    ...
    if (afl->arg_mode && !afl->in_place_resume) {

      init_arg_feedback_channel(afl);
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
      ...
      if (likely(!afl->old_seed_selection)) {
        ...
        u32 prevIdx = 0xffffffff;
        if (likely(afl->queue_cur))
          prevIdx = afl->queue_cur->targetFunctionIdx;
        afl->queue_cur = afl->queue_buf[afl->current_entry];

        if (prevIdx != afl->queue_cur->targetFunctionIdx) {

          be_quiet = 1;
          afl_fsrv_kill(&afl->fsrv);
          afl->fsrv.targetFunctionName = afl->queue_cur->targetFunctionName;
          afl_fsrv_start(&afl->fsrv, afl->argv, &afl->stop_soon,
                      afl->afl_env.afl_debug_child);
          be_quiet = 0;
