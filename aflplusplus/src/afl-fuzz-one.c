...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  if (likely(afl->pending_favored)) {
    ...
    /* Except we only do this if we have already fuzzed every target at least once,
       so that we give each target some attention and only then proceed to ignore
       the least interesting ones. */

    if (afl->firstCycleDone && (afl->queue_cur->fuzz_level || !afl->queue_cur->favored) &&
  ...
  if (unlikely(afl->not_on_tty)) {
    ...
    ACTF(
        "Fuzzing test case #%u with target %s (%u total, %llu crashes saved, "
        "perf_score=%0.0f, exec_us=%llu, hits=%u, map=%u, ascii=%u)...",
        afl->current_entry, afl->queue_cur->targetFunctionName, afl->queued_items, afl->saved_crashes,
