...
u8 common_fuzz_cmplog_stuff(afl_state_t *afl, u8 *out_buf, u32 len) {
  ...
  u8  fault;
  u32 tmp_len = write_to_testcase(afl, (void **)&out_buf, len, 0, afl->queue_cur->targetFunctionName);
  ...
  if (likely(tmp_len)) {
    ...
    len = write_to_testcase(afl, (void **)&out_buf, len, 1, afl->queue_cur->targetFunctionName);
