...
#define SP20 SP10 SP10
  ...
  strncpy(tmp, afl->queue_cur->targetFunctionName, 256);
  SAYF(bV bSTOP " current fun : " cRST "%-20s   " bSTG bV bSTOP "                                     " bSTG bV "\n", tmp);
...
#define IB(i) val_bufs[(i)], sizeof(val_bufs[(i)])
  ...
  // if (afl->total_cal_cycles) {
    ...
    for (size_t i = 0; i < afl->queued_items; i++) {

      if (afl->targetActive[afl->queue_buf[i]->targetFunctionIdx]) {

        avg_us = afl->total_cal_us[afl->queue_buf[i]->targetFunctionIdx] / afl->total_cal_cycles[afl->queue_buf[i]->targetFunctionIdx];
