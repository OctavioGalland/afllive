  ...
  for (iter = 0; iter < afl->foreign_sync_cnt; iter++) {
    ...
    if (afl->foreign_syncs[iter].dir && afl->foreign_syncs[iter].dir[0]) {
      ...
      for (i = 0; i < (u32)nl_cnt; ++i) {
        ...
        char *targetName = getTargetName(nl[i]->d_name);
        u32 len = write_to_testcase(afl, (void **)&mem, st.st_size, 1, targetName);
        fault = fuzz_run_target(afl, &afl->fsrv, afl->fsrv.exec_tmout);
        afl->syncing_party = foreign_name;
        afl->queued_imported += save_if_interesting(afl, mem, len, fault, targetName, getTargetIdx(afl, targetName));
        ck_free(targetName);
        afl->syncing_party = 0;
...
void get_targets(afl_state_t *afl) {

  afl->amountOfTargets = 0;
  char *typeinfoFilename = NULL;
  struct json_value_s *config = parse_json_file(getenv("AH_CONFIG"));
  struct json_object_s *config_root = json_value_as_object(config);
  for (struct json_object_element_s *k = config_root->start; k != NULL; k = k->next) {

    if (!strcmp(k->name->string, "typeinfo_file")) {

      typeinfoFilename = (char*) json_value_as_string(k->value)->string;

    }

  }

  char *blacklist = getenv("AH_BLACKLIST");

  if (typeinfoFilename != NULL) {

    ACTF("Reading target names from %s...", typeinfoFilename);

    // Parse the json string
    struct json_value_s *root = parse_json_file(typeinfoFilename);

    // Traverse the json object and extract function names
    struct json_object_s *objRoot = json_value_as_object(root);
    for (struct json_object_element_s *k = objRoot->start; k != NULL; k = k->next) {

      if (strcmp(k->name->string, "targets") == 0) {

        struct json_object_s *targets_obj = json_value_as_object(k->value);
        for (struct json_object_element_s *t = targets_obj->start; t != NULL && afl->amountOfTargets < MAX_TARGETS; t = t->next) {

          char *delimitedName = malloc(strlen(t->name->string) + 3);
          sprintf(delimitedName, ":%s:", t->name->string);

          if (blacklist && strstr(blacklist, delimitedName)) {

            WARNF("Skipping function %s since it was blacklisted!", t->name->string);

          } else {

            afl->targetNames[afl->amountOfTargets] = strdup(t->name->string);
            afl->amountOfTargets++;
    ...
    if (afl->amountOfTargets > 0) {

      OKF("Successfully read %u function names!", afl->amountOfTargets);

    } else {

      FATAL("All available targets have been blacklisted, aborting!");
...
void get_seeds_from_runtime(afl_state_t *afl) {
  if (afl->fsrv.fsrv_pid) {
    afl_fsrv_kill(&afl->fsrv);
  }
  afl->fsrv.recordArgs = 1;
  afl_fsrv_start(&afl->fsrv, afl->argv, &afl->stop_soon,
                  afl->afl_env.afl_debug_child);

  bool atLeastOneTarget = false;

  fsrv_run_result_t res = fuzz_run_target(afl, &afl->fsrv, afl->fsrv.exec_tmout);

  char *blacklist = getenv("AH_BLACKLIST");
  if (!blacklist) {
    blacklist = "";
  }
  blacklist = strdup(blacklist); // we will deal exclusively with mallocs here

  while (res == FSRV_RUN_CRASH) {
    u8 *entry = afl->arg_map;
    bool failedFunctionFound = false;
    for (u32 i = 0; i < afl->amountOfTargets; i++) {
      u32 serializedArgsLength = *(u32*)(entry + MAX_TARGETNAME_LENGTH);
      bool argsRecorded = 0 != serializedArgsLength;
      if (!argsRecorded) {
        WARNF("Blacklisting '%s', since it was the culprit for a crash during argument recording...", entry);
        size_t newSize = strlen(blacklist) + strlen(entry) + 2 + 1; // 2 for the colons around the name, 1 for the null pointer
        blacklist = realloc(blacklist, newSize);
        strcat(blacklist, ":");
      ...
      entry += MAX_TARGETNAME_LENGTH + 4 + serializedArgsLength;
    }
    ...
    afl_fsrv_start(&afl->fsrv, afl->argv, &afl->stop_soon,
                    afl->afl_env.afl_debug_child);
    res = fuzz_run_target(afl, &afl->fsrv, afl->fsrv.exec_tmout);
  }

  switch (res)
  {
  case FSRV_RUN_OK:
    OKF("Seed recording execution successful!");
    break;
  case FSRV_RUN_TMOUT:
    FATAL("Seed recording timed out!");
    break;
  case FSRV_RUN_CRASH:
    FATAL("Seed recording crashed, maybe check constraints?");
    break;
  default:
    WARNF("No instrumentation/coverage detected during seed recording, let's see how this goes...");
    break;
  }
  ...
  for (u32 i = 0; i < afl->amountOfTargets; i++) {
    ...
    for (u32 j = 0; j < MAX_TARGETS ;j++) {
      ...
      if (strncmp(entry, afl->targetNames[i], MAX_TARGETNAME_LENGTH) == 0) {
        ...
        targetDataLen = entryLen;
        targetData = (u8*)(entry + MAX_TARGETNAME_LENGTH + 4);
        // afl->targetActive[i] = 1;
        atLeastOneTarget = true;
        break;

      } else {

        entry += MAX_TARGETNAME_LENGTH + 4 + entryLen;

      }

    }

    if (targetData == NULL || targetDataLen == 0) {

      WARNF("Target did not inform (or we were unable to retrieve) initial arguments for function %s!", afl->targetNames[i]);

    } else {

      u32 tmpFileNameLen = strlen("/tmp/") + strlen(afl->targetNames[i])  + strlen("-initialXXXXXX");
      u8 *tmpFileName = malloc(tmpFileNameLen + 1);
      snprintf(tmpFileName, tmpFileNameLen + 1, "/tmp/%s-initialXXXXXX", afl->targetNames[i]);

      int tmpFd = mkstemp(tmpFileName);
      ACTF("Writing %u bytes obtained from first run to testcase %s...", targetDataLen, tmpFileName);
      ssize_t written = write(tmpFd, targetData, targetDataLen);
      close(tmpFd);

      if (written == -1 || written != targetDataLen) {

        PFATAL("Error writing first run data to testcase for target %s!", afl->targetNames[i]);

      }

      ACTF("Written %lu bytes. Adding testcase to queue...", written);
      // Add as favored so that it won't be skipped when fuzzing for the first time
      add_favored_to_queue(afl, alloc_printf("%s", tmpFileName), written, 0, afl->targetNames[i], getTargetIdx(afl, afl->targetNames[i]));
  ...
  if (!atLeastOneTarget) {
    ...
    FATAL("Could not capture seed for any target, aborting!");
  ...
  if (nl_cnt) {
    ...
    do {
      ...
      add_to_queue(afl, fn2, st.st_size >= MAX_FILE ? MAX_FILE : st.st_size,
                   passed_det, targetName, getTargetIdx(afl, targetName));
      ck_free(targetName);
      free(nl[i]);
  ...
  for (idx = 0; idx < afl->queued_items; idx++) {
    ...
    switch (res) {
      ...
      case FSRV_RUN_OK:
        ...
        afl->targetActive[q->targetFunctionIdx] = 1;
...
#undef MSG_FORK_ON_APPLE
  ...
  for (idx = 0; idx < afl->queued_items; idx++) {
    ...
    for (i = idx + 1;
      ...
      if (p->exec_cksum == q->exec_cksum && q->targetFunctionIdx == p->targetFunctionIdx) {
  ...
  if (afl->arg_mode && afl->amountOfTargets > 0 && !afl->targetActive[0]) {
...
void pivot_inputs(afl_state_t *afl) {
  ...
  for (i = 0; i < afl->queued_items && likely(afl->queue_buf[i]); i++) {
    ...
    //   if (!strcmp(afl->targetNames[j], q->targetFunctionName)) {

    //     afl->targetActive[j] = 1;
    ...
    if (!strncmp(orsl, CASE_PREFIX, 3) &&
        sscanf(orsl + 3, "%06u", &orig_id) == 1 && orig_id == id) {
      ...
      nfn = alloc_printf("%s/queue/%s-id:%06u,time:0,execs:%llu,orig:%s",
                         afl->out_dir, q->targetFunctionName, id, afl->fsrv.total_execs, use_name);

      if (strlen(nfn) > 256) {

        free(nfn);
        nfn = alloc_printf("%s/queue/%s-id:%06u,time:0,execs:%llu,orig:long",
                           afl->out_dir, q->targetFunctionName, id, afl->fsrv.total_execs);

      }
#else

      nfn = alloc_printf("%s/queue/%s-id_%06u", afl->out_dir, q->targetFunctionName, id);
...
/* A helper function for handle_existing_out_dir(), deleting all filed
    with a given prefix AFTER the specified character */

static u8 delete_files_wildcard(u8 *path, u8 *prefix, u8 after) {
  ...
  while ((d_ent = readdir(d))) {
    ...
    if (d_ent->d_name[0] != '.' &&
        (!prefix || (strchr(d_ent->d_name, after) != NULL && !strncmp(strchr(d_ent->d_name, after) + 1, prefix, strlen(prefix))))) {
...
void nuke_resume_dir(afl_state_t *afl) {
  ...
  if (delete_files_wildcard(fn, CASE_PREFIX, '-')) { goto dir_cleanup_failed; }
  ck_free(fn);
...
void init_arg_feedback_channel(afl_state_t *afl) {
  ...
  u8 *shm_str = alloc_printf("%d", afl->arg_shm_fuzz->shm_id);
  setenv(SHM_ARG_FUZZ_ENV_VAR, shm_str, 1);
  ck_free(shm_str);
